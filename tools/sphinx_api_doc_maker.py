...
def main(
    ...
    Main function.
    '''
    doc_dir = os.path.abspath(os.path.join(gabra_converter.path, '..', '..', 'docs', 'source'))
    ...
    try:
        ...
        pass
    try:
        os.remove(os.path.join(doc_dir, 'gabra_converter.rst'))
    except FileNotFoundError:
        pass
    try:
        shutil.rmtree(os.path.join(doc_dir, 'gabra_converter'))
    except FileNotFoundError:
        pass

    with open(
        os.path.join(gabra_converter.path, '..', '..', 'docs', 'source', 'index.rst'),
        'w', encoding='utf-8',
    ) as f:
        print('''\
Welcome to Ġabra Converter API's documentation!
===============================================
...
''' + (
       ''
       if set(os.listdir(os.path.join(gabra_converter.path))) - {'__pycache__'} == {'__init__.py'}
       else '   gabra_converter'
   ) + '''
...
''', file=f)
    ...
    generate(
        os.path.abspath(os.path.join(gabra_converter.path, '..')),
        doc_dir,
        [],
    )
