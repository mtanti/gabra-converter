...
class Test(unittest.TestCase):
    ...
    def test_archive_extraction(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_path:
            extract_archived_files(
                os.path.join(
                    gabra_converter.path, 'tests', 'archive_extractor', 'mock_dump.tar.gz'
                ),
                tmp_path,
            )
            ...
            self.assertEqual(
                os.listdir(os.path.join(tmp_path, 'tmp')),
                ['gabra'],
            )
            self.assertEqual(
                os.listdir(os.path.join(tmp_path, 'tmp', 'gabra')),
                [
                    'lexemes.bson',
    ...
    def test_bson_conversion(self) -> None:
        '''
        ...
        with tempfile.TemporaryDirectory() as tmp_path:
            ...
            with open(
                os.path.join(
                    gabra_converter.path, 'tests', 'archive_extractor', 'mock_lexemes.jsonl'
                ),
                'r', encoding='utf-8',
            ) as f:
            ...
            with open(
                os.path.join(
                    gabra_converter.path, 'tests', 'archive_extractor', 'mock_wordforms.jsonl'
                ),
                'r', encoding='utf-8',
            ) as f:
                text_expected = f.read()
